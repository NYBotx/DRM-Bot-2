...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7765337545:AAGSTEL2M4QoKf3mUFhaZQdlu3MRN2dA1C8")
    API_ID = int(os.environ.get("API_ID",  "24720215"))
    API_HASH = os.environ.get("API_HASH", "c0d3395590fecba19985f95d6300785e")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '5901568800').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002282526453').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002282526453")
